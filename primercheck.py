	...
	for data in imgListCam:
		if data['CYCLE']==str(numCycle):
			...
			if (os.path.exists(pathpng) == True):
				'''
				overlay = cv2.imread(pathpng, 1)
				overlay = cv2.resize(overlay, (2448, 2048))

				background=img
				#background = cv2.cvtColor(img, cv2.COLOR_BGR2BGRA)
				#overlay = cv2.cvtColor(overlay, cv2.COLOR_BGR2BGRA)

				added_image = cv2.addWeighted(background,0.1,image_bgra,0.9,0.5)
				
				img = cv2.cvtColor(added_image, cv2.COLOR_BGRA2BGR)
				'''

				overlay = Image.open('camTest/cam1/'+data['PATH'])
